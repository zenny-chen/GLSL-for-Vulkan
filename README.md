...
这返回了一个类型 **`int`**。如果一个数组已经被显式地指明大小，那么由 **`length()`** 方法所返回的值是一个常量表达式。如果一个数组没有被显式地指明大小，并且它是一个 shader storage block 所声明的最后的成员，那么返回值将不是一个常量表达式，并且将在运行时基于那个 block 所提供存储的缓存对象的大小进行确定。这种数组是运行时确定大小的。对于运行时确定大小的数组，由 **`length()`** 方法所返回的值将是未定义的，如果该数组被包含在 shader storage blocks 的一个数组中，且用一个小于零或大于等于该数组中 blocks 个数的非常量表达式进行索引。

**`length()`** 方法不能对这么一个数组进行调用，该数组不在运行时指明大小，并且也尚未被显式地指明大小；这将会导致一个编译时错误。

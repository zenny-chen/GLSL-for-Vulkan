...
**`uniform`** 限定符被用于声明全局变量，它们的值跨整个正被处理的图元都是相同的。所有 **`uniform`** 变量都是只读的，并且在外部要么在连接时、要么通过API在外部被初始化。连接时初始值要么是变量初始化器的值，如果存在；要么是0，如果没有初始化器。隐含类型不能具有初始化器，否则会导致编译时错误。当目标API为Vulkan时，在一个 block 外部声明 **`uniform`** 变量变量则会产生编译时错误。

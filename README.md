...
图像变量是对一维、二维、或三维图像的句柄，对应于绑定在一个图像单元的一单个细节度的纹理图像的全部或一部分。对于每种纹理目标，以及对于每种浮点、带符号整数、还有无符号整数数据类型，都有不同的图像变量类型。图像访问应该使用匹配于纹理目标的一种图像类型，其细节度绑定于图像单元中，或是对于3D或阵列图像的非分层绑定，应该使用匹配于该层图像维度的图像类型。（即，一个3D、2D阵列、立方体、或立方体阵列的一个图层，应该使用 **`image2D`**，一个1D阵列的一个图层应该使用 **`image1D`**，而对于2D多重采样阵列的一个图层应该使用 **`image2DMS`**。）如果图像目标类型不匹配于此方式下所绑定的图像，如果数据类型不匹配于所绑定的图像，或如果格式 **`layout`** 限定符不匹配于由OpenGL规格说明中“纹理图像加载与存储（Texture Image Loads and Stores）”这一章所描述的图像单元格式，那么图像访问的结果是未定义的，但不能导致程序终结。
